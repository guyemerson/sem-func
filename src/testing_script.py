...
from core.__config__.filepath import AUX_DIR, OUT_DIR
...
pred_name = new.model.pred_name
pred_freq = new.model.freq
...
for pair, score in zip(*simlex[0]):
    if all(raw_freq[freq_lookup['n'].get(x,0)] > 1000 for x in pair):
